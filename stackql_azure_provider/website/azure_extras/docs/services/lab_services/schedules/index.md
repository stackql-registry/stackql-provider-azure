...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_extras.lab_services.schedules" /></td></tr>
</tbody></table>
...
timeZoneId,
type
FROM azure_extras.lab_services.schedules
...
```sql
INSERT INTO azure_extras.lab_services.schedules (
properties,
resource_group_name,
lab_name,
schedule_name,
...
```sql
UPDATE azure_extras.lab_services.schedules
SET 
...
```sql
REPLACE azure_extras.lab_services.schedules
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure_extras.lab_services.schedules
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND lab_name = '{{ lab_name }}' --required
AND schedule_name = '{{ schedule_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
