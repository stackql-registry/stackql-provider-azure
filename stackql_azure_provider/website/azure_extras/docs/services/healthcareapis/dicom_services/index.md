...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_extras.healthcareapis.dicom_services" /></td></tr>
</tbody></table>
...
tags,
type
FROM azure_extras.healthcareapis.dicom_services
...
```sql
INSERT INTO azure_extras.healthcareapis.dicom_services (
properties,
tags,
location,
etag,
...
```sql
UPDATE azure_extras.healthcareapis.dicom_services
SET 
tags = '{{ tags }}',
identity = '{{ identity }}'
WHERE 
...
```sql
REPLACE azure_extras.healthcareapis.dicom_services
SET 
...
```sql
DELETE FROM azure_extras.healthcareapis.dicom_services
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND dicom_service_name = '{{ dicom_service_name }}' --required
AND workspace_name = '{{ workspace_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
