...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_extras.rdbms.virtual_network_rules" /></td></tr>
</tbody></table>
...
FROM azure_extras.rdbms.virtual_network_rules
...
```sql
INSERT INTO azure_extras.rdbms.virtual_network_rules (
properties,
resource_group_name,
server_name,
...
```sql
REPLACE azure_extras.rdbms.virtual_network_rules
SET 
...
```sql
DELETE FROM azure_extras.rdbms.virtual_network_rules
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND server_name = '{{ server_name }}' --required
AND virtual_network_rule_name = '{{ virtual_network_rule_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
