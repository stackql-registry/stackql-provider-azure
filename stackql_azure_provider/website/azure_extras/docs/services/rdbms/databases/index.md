...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_extras.rdbms.databases" /></td></tr>
</tbody></table>
...
collation,
type
FROM azure_extras.rdbms.databases
...
```sql
INSERT INTO azure_extras.rdbms.databases (
properties,
resource_group_name,
server_name,
database_name,
...
```sql
REPLACE azure_extras.rdbms.databases
SET 
...
```sql
DELETE FROM azure_extras.rdbms.databases
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND server_name = '{{ server_name }}' --required
AND database_name = '{{ database_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
