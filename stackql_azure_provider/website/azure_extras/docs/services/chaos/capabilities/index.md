...
type,
urn
FROM azure_extras.chaos.capabilities
...
```sql
INSERT INTO azure_extras.chaos.capabilities (
properties,
resource_group_name,
...
```sql
REPLACE azure_extras.chaos.capabilities
SET 
...
```sql
DELETE FROM azure_extras.chaos.capabilities
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND parent_provider_namespace = '{{ parent_provider_namespace }}' --required
AND parent_resource_type = '{{ parent_resource_type }}' --required
AND parent_resource_name = '{{ parent_resource_name }}' --required
