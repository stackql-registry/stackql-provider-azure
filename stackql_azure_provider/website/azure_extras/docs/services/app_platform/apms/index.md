...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_extras.app_platform.apms" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_extras.app_platform.apms
...
```sql
INSERT INTO azure_extras.app_platform.apms (
properties,
resource_group_name,
service_name,
apm_name,
...
```sql
REPLACE azure_extras.app_platform.apms
SET 
properties = '{{ properties }}'
WHERE 
...
```sql
DELETE FROM azure_extras.app_platform.apms
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND apm_name = '{{ apm_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure_extras.app_platform.apms.list_secret_keys 
@resource_group_name='{{ resource_group_name }}' --required, 
