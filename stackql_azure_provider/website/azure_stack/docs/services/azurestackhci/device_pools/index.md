...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_stack.azurestackhci.device_pools" /></td></tr>
</tbody></table>
...
tags,
type
FROM azure_stack.azurestackhci.device_pools
...
```sql
INSERT INTO azure_stack.azurestackhci.device_pools (
tags,
location,
properties,
identity,
...
```sql
UPDATE azure_stack.azurestackhci.device_pools
SET 
tags = '{{ tags }}',
identity = '{{ identity }}'
WHERE 
...
```sql
REPLACE azure_stack.azurestackhci.device_pools
SET 
tags = '{{ tags }}',
location = '{{ location }}',
...
```sql
DELETE FROM azure_stack.azurestackhci.device_pools
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND device_pool_name = '{{ device_pool_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
...
```sql
EXEC azure_stack.azurestackhci.device_pools.claim_devices 
@resource_group_name='{{ resource_group_name }}' --required, 
...
```sql
EXEC azure_stack.azurestackhci.device_pools.release_devices 
@resource_group_name='{{ resource_group_name }}' --required, 
@device_pool_name='{{ device_pool_name }}' --required, 
@subscription_id='{{ subscription_id }}' --required 
@@json=
