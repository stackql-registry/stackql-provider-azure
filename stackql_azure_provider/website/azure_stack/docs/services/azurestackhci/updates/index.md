...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_stack.azurestackhci.updates" /></td></tr>
</tbody></table>
...
version
FROM azure_stack.azurestackhci.updates
...
```sql
DELETE FROM azure_stack.azurestackhci.updates
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND update_name = '{{ update_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure_stack.azurestackhci.updates.put 
...
```sql
EXEC azure_stack.azurestackhci.updates.post 
@resource_group_name='{{ resource_group_name }}' --required, 
@cluster_name='{{ cluster_name }}' --required, 
@update_name='{{ update_name }}' --required, 
@subscription_id='{{ subscription_id }}' --required
...
```sql
EXEC azure_stack.azurestackhci.updates.prepare 
@resource_group_name='{{ resource_group_name }}' --required, 
@cluster_name='{{ cluster_name }}' --required, 
@update_name='{{ update_name }}' --required, 
@subscription_id='{{ subscription_id }}' --required
