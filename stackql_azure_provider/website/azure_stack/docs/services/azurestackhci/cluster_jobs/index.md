...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_stack.azurestackhci.cluster_jobs" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_stack.azurestackhci.cluster_jobs
...
```sql
INSERT INTO azure_stack.azurestackhci.cluster_jobs (
properties,
resource_group_name,
cluster_name,
jobs_name,
...
```sql
REPLACE azure_stack.azurestackhci.cluster_jobs
SET 
...
```sql
DELETE FROM azure_stack.azurestackhci.cluster_jobs
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND jobs_name = '{{ jobs_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
