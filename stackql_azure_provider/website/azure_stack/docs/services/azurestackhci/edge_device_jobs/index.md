...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_stack.azurestackhci.edge_device_jobs" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_stack.azurestackhci.edge_device_jobs
...
```sql
INSERT INTO azure_stack.azurestackhci.edge_device_jobs (
kind,
resource_uri,
jobs_name,
edge_device_name
...
```sql
REPLACE azure_stack.azurestackhci.edge_device_jobs
SET 
kind = '{{ kind }}'
WHERE 
...
```sql
DELETE FROM azure_stack.azurestackhci.edge_device_jobs
WHERE resource_uri = '{{ resource_uri }}' --required
AND jobs_name = '{{ jobs_name }}' --required
AND edge_device_name = '{{ edge_device_name }}' --required
;
