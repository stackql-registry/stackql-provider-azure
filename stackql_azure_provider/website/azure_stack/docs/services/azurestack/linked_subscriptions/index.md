...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_stack.azurestack.linked_subscriptions" /></td></tr>
</tbody></table>
...
tags,
type
FROM azure_stack.azurestack.linked_subscriptions
...
```sql
INSERT INTO azure_stack.azurestack.linked_subscriptions (
location,
properties,
resource_group,
...
```sql
UPDATE azure_stack.azurestack.linked_subscriptions
SET 
location = '{{ location }}',
...
```sql
REPLACE azure_stack.azurestack.linked_subscriptions
SET 
location = '{{ location }}',
properties = '{{ properties }}'
WHERE 
...
```sql
DELETE FROM azure_stack.azurestack.linked_subscriptions
WHERE resource_group = '{{ resource_group }}' --required
AND linked_subscription_name = '{{ linked_subscription_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
