...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_isv.newrelicobservability.tag_rules" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_isv.newrelicobservability.tag_rules
...
```sql
INSERT INTO azure_isv.newrelicobservability.tag_rules (
properties,
resource_group_name,
monitor_name,
rule_set_name,
...
```sql
UPDATE azure_isv.newrelicobservability.tag_rules
SET 
...
```sql
REPLACE azure_isv.newrelicobservability.tag_rules
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure_isv.newrelicobservability.tag_rules
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND monitor_name = '{{ monitor_name }}' --required
AND rule_set_name = '{{ rule_set_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
