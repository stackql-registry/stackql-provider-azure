...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_isv.mongodb_atlas.clusters" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_isv.mongodb_atlas.clusters
...
```sql
INSERT INTO azure_isv.mongodb_atlas.clusters (
properties,
resource_group_name,
organization_name,
project_name,
...
```sql
REPLACE azure_isv.mongodb_atlas.clusters
SET 
...
```sql
DELETE FROM azure_isv.mongodb_atlas.clusters
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND organization_name = '{{ organization_name }}' --required
AND project_name = '{{ project_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
