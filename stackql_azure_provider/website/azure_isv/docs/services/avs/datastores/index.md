...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_isv.avs.datastores" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure_isv.avs.datastores (
properties,
resource_group_name,
private_cloud_name,
cluster_name,
...
```sql
REPLACE azure_isv.avs.datastores
SET 
...
```sql
DELETE FROM azure_isv.avs.datastores
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_cloud_name = '{{ private_cloud_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND datastore_name = '{{ datastore_name }}' --required
