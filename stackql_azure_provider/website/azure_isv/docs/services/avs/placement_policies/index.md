...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_isv.avs.placement_policies" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_isv.avs.placement_policies
...
```sql
INSERT INTO azure_isv.avs.placement_policies (
properties,
resource_group_name,
private_cloud_name,
cluster_name,
...
```sql
UPDATE azure_isv.avs.placement_policies
SET 
...
```sql
REPLACE azure_isv.avs.placement_policies
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure_isv.avs.placement_policies
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_cloud_name = '{{ private_cloud_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND placement_policy_name = '{{ placement_policy_name }}' --required
