...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_isv.avs.global_reach_connections" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_isv.avs.global_reach_connections
...
```sql
INSERT INTO azure_isv.avs.global_reach_connections (
properties,
resource_group_name,
private_cloud_name,
...
```sql
REPLACE azure_isv.avs.global_reach_connections
SET 
...
```sql
DELETE FROM azure_isv.avs.global_reach_connections
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_cloud_name = '{{ private_cloud_name }}' --required
AND global_reach_connection_name = '{{ global_reach_connection_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
