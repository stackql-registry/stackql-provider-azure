...
<tr><td><b>Id</b></td><td><CopyableCode code="azure_isv.avs.pure_storage_policies" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure_isv.avs.pure_storage_policies
...
```sql
INSERT INTO azure_isv.avs.pure_storage_policies (
properties,
resource_group_name,
private_cloud_name,
storage_policy_name,
...
```sql
REPLACE azure_isv.avs.pure_storage_policies
SET 
...
```sql
DELETE FROM azure_isv.avs.pure_storage_policies
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_cloud_name = '{{ private_cloud_name }}' --required
AND storage_policy_name = '{{ storage_policy_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
