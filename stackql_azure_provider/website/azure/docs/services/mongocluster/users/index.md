...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.mongocluster.users" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.mongocluster.users (
properties,
resource_group_name,
mongo_cluster_name,
user_name,
...
```sql
REPLACE azure.mongocluster.users
SET 
...
```sql
DELETE FROM azure.mongocluster.users
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND mongo_cluster_name = '{{ mongo_cluster_name }}' --required
AND user_name = '{{ user_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
