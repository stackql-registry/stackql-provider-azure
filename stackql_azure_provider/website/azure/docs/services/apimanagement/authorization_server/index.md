...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.apimanagement.authorization_server" /></td></tr>
</tbody></table>
...
useInTestConsole
FROM azure.apimanagement.authorization_server
...
```sql
INSERT INTO azure.apimanagement.authorization_server (
properties,
resource_group_name,
service_name,
authsid,
...
```sql
UPDATE azure.apimanagement.authorization_server
SET 
...
```sql
REPLACE azure.apimanagement.authorization_server
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure.apimanagement.authorization_server
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND authsid = '{{ authsid }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure.apimanagement.authorization_server.get_entity_tag 
@resource_group_name='{{ resource_group_name }}' --required, 
...
```sql
EXEC azure.apimanagement.authorization_server.list_secrets 
@resource_group_name='{{ resource_group_name }}' --required, 
@service_name='{{ service_name }}' --required, 
@authsid='{{ authsid }}' --required, 
@subscription_id='{{ subscription_id }}' --required
