...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.apimanagement.group_user" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.apimanagement.group_user
...
```sql
INSERT INTO azure.apimanagement.group_user (
resource_group_name,
service_name,
group_id,
user_id,
...
```sql
DELETE FROM azure.apimanagement.group_user
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND group_id = '{{ group_id }}' --required
...
```sql
EXEC azure.apimanagement.group_user.check_entity_exists 
