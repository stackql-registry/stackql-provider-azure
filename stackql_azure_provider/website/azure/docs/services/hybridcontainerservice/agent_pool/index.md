...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.hybridcontainerservice.agent_pool" /></td></tr>
</tbody></table>
...
type,
vmSize
FROM azure.hybridcontainerservice.agent_pool
...
```sql
INSERT INTO azure.hybridcontainerservice.agent_pool (
properties,
tags,
extendedLocation,
...
```sql
REPLACE azure.hybridcontainerservice.agent_pool
SET 
...
```sql
DELETE FROM azure.hybridcontainerservice.agent_pool
WHERE connected_cluster_resource_uri = '{{ connected_cluster_resource_uri }}' --required
AND agent_pool_name = '{{ agent_pool_name }}' --required
;
```
