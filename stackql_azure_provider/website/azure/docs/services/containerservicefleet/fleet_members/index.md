...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerservicefleet.fleet_members" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.containerservicefleet.fleet_members
...
```sql
INSERT INTO azure.containerservicefleet.fleet_members (
properties,
resource_group_name,
fleet_name,
fleet_member_name,
...
```sql
UPDATE azure.containerservicefleet.fleet_members
SET 
...
```sql
DELETE FROM azure.containerservicefleet.fleet_members
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND fleet_name = '{{ fleet_name }}' --required
AND fleet_member_name = '{{ fleet_member_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
