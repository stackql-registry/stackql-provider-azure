...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.costmanagement.cost_allocation_rules" /></td></tr>
</tbody></table>
...
type,
updatedDate
FROM azure.costmanagement.cost_allocation_rules
...
```sql
INSERT INTO azure.costmanagement.cost_allocation_rules (
properties,
billing_account_id,
rule_name
)
...
```sql
REPLACE azure.costmanagement.cost_allocation_rules
SET 
...
```sql
DELETE FROM azure.costmanagement.cost_allocation_rules
WHERE billing_account_id = '{{ billing_account_id }}' --required
AND rule_name = '{{ rule_name }}' --required
;
```
...
```sql
EXEC azure.costmanagement.cost_allocation_rules.check_name_availability 
@billing_account_id='{{ billing_account_id }}' --required 
@@json=
'{
"name": "{{ name }}", 
