...
kubeconfigs
FROM azure.hybrid_kubernetes.connected_cluster
...
```sql
INSERT INTO azure.hybrid_kubernetes.connected_cluster (
tags,
location,
properties,
identity,
...
```sql
REPLACE azure.hybrid_kubernetes.connected_cluster
SET 
tags = '{{ tags }}',
location = '{{ location }}',
...
```sql
DELETE FROM azure.hybrid_kubernetes.connected_cluster
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
...
```sql
EXEC azure.hybrid_kubernetes.connected_cluster.update_async 
@resource_group_name='{{ resource_group_name }}' --required, 
