...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.imagebuilder.triggers" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.imagebuilder.triggers
...
```sql
INSERT INTO azure.imagebuilder.triggers (
properties,
resource_group_name,
image_template_name,
trigger_name,
...
```sql
REPLACE azure.imagebuilder.triggers
SET 
...
```sql
DELETE FROM azure.imagebuilder.triggers
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND image_template_name = '{{ image_template_name }}' --required
AND trigger_name = '{{ trigger_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
