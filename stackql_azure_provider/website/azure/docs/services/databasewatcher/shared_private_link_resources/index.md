...
systemData,
type
FROM azure.databasewatcher.shared_private_link_resources
...
```sql
INSERT INTO azure.databasewatcher.shared_private_link_resources (
properties,
resource_group_name,
watcher_name,
...
```sql
DELETE FROM azure.databasewatcher.shared_private_link_resources
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND watcher_name = '{{ watcher_name }}' --required
AND shared_private_link_resource_name = '{{ shared_private_link_resource_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
