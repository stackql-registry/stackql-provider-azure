...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.durabletask.task_hubs" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.durabletask.task_hubs
...
```sql
INSERT INTO azure.durabletask.task_hubs (
properties,
resource_group_name,
scheduler_name,
task_hub_name,
...
```sql
REPLACE azure.durabletask.task_hubs
SET 
...
```sql
DELETE FROM azure.durabletask.task_hubs
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND scheduler_name = '{{ scheduler_name }}' --required
AND task_hub_name = '{{ task_hub_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
