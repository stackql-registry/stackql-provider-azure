...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.resource_policy.policy_assignments" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.resource_policy.policy_assignments
...
```sql
INSERT INTO azure.resource_policy.policy_assignments (
properties,
location,
identity,
scope,
...
```sql
UPDATE azure.resource_policy.policy_assignments
SET 
...
```sql
DELETE FROM azure.resource_policy.policy_assignments
WHERE scope = '{{ scope }}' --required
AND policy_assignment_name = '{{ policy_assignment_name }}' --required
;
```
