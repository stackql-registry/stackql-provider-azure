...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.durable_task.retention_policies" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.durable_task.retention_policies
...
```sql
INSERT INTO azure.durable_task.retention_policies (
properties,
resource_group_name,
scheduler_name,
subscription_id
...
```sql
UPDATE azure.durable_task.retention_policies
SET 
...
```sql
REPLACE azure.durable_task.retention_policies
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure.durable_task.retention_policies
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND scheduler_name = '{{ scheduler_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
...
```sql
EXEC azure.durable_task.retention_policies.list_by_scheduler 
@resource_group_name='{{ resource_group_name }}' --required, 
