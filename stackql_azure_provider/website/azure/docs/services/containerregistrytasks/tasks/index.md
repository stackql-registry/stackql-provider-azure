...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerregistrytasks.tasks" /></td></tr>
</tbody></table>
...
trigger,
type
FROM azure.containerregistrytasks.tasks
...
```sql
INSERT INTO azure.containerregistrytasks.tasks (
tags,
location,
properties,
identity,
...
```sql
UPDATE azure.containerregistrytasks.tasks
SET 
identity = '{{ identity }}',
...
```sql
DELETE FROM azure.containerregistrytasks.tasks
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND registry_name = '{{ registry_name }}' --required
AND task_name = '{{ task_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure.containerregistrytasks.tasks.get_details 
@resource_group_name='{{ resource_group_name }}' --required, 
