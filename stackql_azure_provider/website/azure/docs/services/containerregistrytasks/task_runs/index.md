...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerregistrytasks.task_runs" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.containerregistrytasks.task_runs
...
```sql
INSERT INTO azure.containerregistrytasks.task_runs (
properties,
identity,
location,
resource_group_name,
...
```sql
UPDATE azure.containerregistrytasks.task_runs
SET 
identity = '{{ identity }}',
...
```sql
DELETE FROM azure.containerregistrytasks.task_runs
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND registry_name = '{{ registry_name }}' --required
AND task_run_name = '{{ task_run_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure.containerregistrytasks.task_runs.get_details 
@resource_group_name='{{ resource_group_name }}' --required, 
