...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workloadorchestration.schema_versions" /></td></tr>
</tbody></table>
...
type,
value
FROM azure.workloadorchestration.schema_versions
...
```sql
INSERT INTO azure.workloadorchestration.schema_versions (
properties,
resource_group_name,
schema_name,
schema_version_name,
...
```sql
UPDATE azure.workloadorchestration.schema_versions
SET 
...
```sql
REPLACE azure.workloadorchestration.schema_versions
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure.workloadorchestration.schema_versions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND schema_name = '{{ schema_name }}' --required
AND schema_version_name = '{{ schema_version_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
