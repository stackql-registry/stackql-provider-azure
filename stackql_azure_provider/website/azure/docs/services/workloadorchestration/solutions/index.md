...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workloadorchestration.solutions" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.workloadorchestration.solutions
...
```sql
INSERT INTO azure.workloadorchestration.solutions (
properties,
extendedLocation,
resource_group_name,
target_name,
...
```sql
UPDATE azure.workloadorchestration.solutions
SET 
...
```sql
REPLACE azure.workloadorchestration.solutions
SET 
properties = '{{ properties }}',
extendedLocation = '{{ extendedLocation }}'
WHERE 
...
```sql
DELETE FROM azure.workloadorchestration.solutions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND target_name = '{{ target_name }}' --required
AND solution_name = '{{ solution_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
