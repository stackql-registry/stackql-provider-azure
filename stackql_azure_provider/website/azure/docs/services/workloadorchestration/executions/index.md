...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workloadorchestration.executions" /></td></tr>
</tbody></table>
...
type,
workflowVersionId
FROM azure.workloadorchestration.executions
...
```sql
INSERT INTO azure.workloadorchestration.executions (
properties,
extendedLocation,
resource_group_name,
context_name,
...
```sql
UPDATE azure.workloadorchestration.executions
SET 
...
```sql
REPLACE azure.workloadorchestration.executions
SET 
properties = '{{ properties }}',
extendedLocation = '{{ extendedLocation }}'
WHERE 
...
```sql
DELETE FROM azure.workloadorchestration.executions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND context_name = '{{ context_name }}' --required
AND workflow_name = '{{ workflow_name }}' --required
AND version_name = '{{ version_name }}' --required
