...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workloadorchestration.workflows" /></td></tr>
</tbody></table>
...
type,
workflowTemplateId
FROM azure.workloadorchestration.workflows
...
```sql
INSERT INTO azure.workloadorchestration.workflows (
properties,
extendedLocation,
resource_group_name,
context_name,
...
```sql
UPDATE azure.workloadorchestration.workflows
SET 
...
```sql
REPLACE azure.workloadorchestration.workflows
SET 
properties = '{{ properties }}',
extendedLocation = '{{ extendedLocation }}'
WHERE 
...
```sql
DELETE FROM azure.workloadorchestration.workflows
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND context_name = '{{ context_name }}' --required
AND workflow_name = '{{ workflow_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
