...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workloadorchestration.workflow_versions" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.workloadorchestration.workflow_versions
...
```sql
INSERT INTO azure.workloadorchestration.workflow_versions (
properties,
extendedLocation,
resource_group_name,
context_name,
...
```sql
UPDATE azure.workloadorchestration.workflow_versions
SET 
...
```sql
REPLACE azure.workloadorchestration.workflow_versions
SET 
properties = '{{ properties }}',
extendedLocation = '{{ extendedLocation }}'
WHERE 
...
```sql
DELETE FROM azure.workloadorchestration.workflow_versions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND context_name = '{{ context_name }}' --required
AND workflow_name = '{{ workflow_name }}' --required
AND version_name = '{{ version_name }}' --required
