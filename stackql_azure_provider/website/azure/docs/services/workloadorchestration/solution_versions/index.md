...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workloadorchestration.solution_versions" /></td></tr>
</tbody></table>
...
type
FROM azure.workloadorchestration.solution_versions
...
```sql
INSERT INTO azure.workloadorchestration.solution_versions (
properties,
extendedLocation,
resource_group_name,
target_name,
...
```sql
UPDATE azure.workloadorchestration.solution_versions
SET 
...
```sql
REPLACE azure.workloadorchestration.solution_versions
SET 
properties = '{{ properties }}',
extendedLocation = '{{ extendedLocation }}'
WHERE 
...
```sql
DELETE FROM azure.workloadorchestration.solution_versions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND target_name = '{{ target_name }}' --required
AND solution_name = '{{ solution_name }}' --required
AND solution_version_name = '{{ solution_version_name }}' --required
