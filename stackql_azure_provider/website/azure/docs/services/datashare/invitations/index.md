...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.datashare.invitations" /></td></tr>
</tbody></table>
...
userEmail,
userName
FROM azure.datashare.invitations
...
```sql
INSERT INTO azure.datashare.invitations (
properties,
resource_group_name,
account_name,
share_name,
...
```sql
DELETE FROM azure.datashare.invitations
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND account_name = '{{ account_name }}' --required
AND share_name = '{{ share_name }}' --required
AND invitation_name = '{{ invitation_name }}' --required
