...
type,
version
FROM azure.api_management.content_type
...
```sql
INSERT INTO azure.api_management.content_type (
properties,
resource_group_name,
service_name,
content_type_id,
...
```sql
REPLACE azure.api_management.content_type
SET 
...
```sql
DELETE FROM azure.api_management.content_type
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND content_type_id = '{{ content_type_id }}' --required
AND subscription_id = '{{ subscription_id }}' --required
