...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.api_management.product_api_link" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.api_management.product_api_link
...
```sql
INSERT INTO azure.api_management.product_api_link (
properties,
resource_group_name,
service_name,
product_id,
...
```sql
REPLACE azure.api_management.product_api_link
SET 
...
```sql
DELETE FROM azure.api_management.product_api_link
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND product_id = '{{ product_id }}' --required
AND api_link_id = '{{ api_link_id }}' --required
