...
type,
updatedDateTime
FROM azure.api_management.workspace_api_release
...
```sql
INSERT INTO azure.api_management.workspace_api_release (
properties,
resource_group_name,
service_name,
workspace_id,
...
```sql
UPDATE azure.api_management.workspace_api_release
SET 
...
```sql
REPLACE azure.api_management.workspace_api_release
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure.api_management.workspace_api_release
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND workspace_id = '{{ workspace_id }}' --required
AND api_id = '{{ api_id }}' --required
...
```sql
EXEC azure.api_management.workspace_api_release.get_entity_tag 
@resource_group_name='{{ resource_group_name }}' --required, 
