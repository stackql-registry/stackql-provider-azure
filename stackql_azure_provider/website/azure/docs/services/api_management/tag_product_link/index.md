...
systemData,
type
FROM azure.api_management.tag_product_link
...
```sql
INSERT INTO azure.api_management.tag_product_link (
properties,
resource_group_name,
service_name,
tag_id,
...
```sql
REPLACE azure.api_management.tag_product_link
SET 
...
```sql
DELETE FROM azure.api_management.tag_product_link
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND tag_id = '{{ tag_id }}' --required
AND product_link_id = '{{ product_link_id }}' --required
