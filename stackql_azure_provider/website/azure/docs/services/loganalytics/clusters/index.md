...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.loganalytics.clusters" /></td></tr>
</tbody></table>
...
tags,
type
FROM azure.loganalytics.clusters
...
```sql
INSERT INTO azure.loganalytics.clusters (
tags,
location,
properties,
identity,
...
```sql
UPDATE azure.loganalytics.clusters
SET 
...
```sql
REPLACE azure.loganalytics.clusters
SET 
tags = '{{ tags }}',
location = '{{ location }}',
properties = '{{ properties }}',
...
```sql
DELETE FROM azure.loganalytics.clusters
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
