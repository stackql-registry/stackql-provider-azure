...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.privatedns.record_sets" /></td></tr>
</tbody></table>
...
txtRecords,
type
FROM azure.privatedns.record_sets
...
```sql
INSERT INTO azure.privatedns.record_sets (
etag,
properties,
resource_group_name,
private_zone_name,
...
```sql
UPDATE azure.privatedns.record_sets
SET 
etag = '{{ etag }}',
...
```sql
REPLACE azure.privatedns.record_sets
SET 
etag = '{{ etag }}',
properties = '{{ properties }}'
WHERE 
...
```sql
DELETE FROM azure.privatedns.record_sets
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_zone_name = '{{ private_zone_name }}' --required
AND record_type = '{{ record_type }}' --required
AND relative_record_set_name = '{{ relative_record_set_name }}' --required
