...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.managed_ops.managed_ops" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.managed_ops.managed_ops
...
```sql
INSERT INTO azure.managed_ops.managed_ops (
properties,
managed_ops_name,
subscription_id
)
...
```sql
UPDATE azure.managed_ops.managed_ops
SET 
...
```sql
REPLACE azure.managed_ops.managed_ops
SET 
properties = '{{ properties }}'
WHERE 
managed_ops_name = '{{ managed_ops_name }}' --required
...
```sql
DELETE FROM azure.managed_ops.managed_ops
WHERE managed_ops_name = '{{ managed_ops_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
```
