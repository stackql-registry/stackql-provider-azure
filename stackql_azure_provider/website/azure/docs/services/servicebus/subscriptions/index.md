...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.servicebus.subscriptions" /></td></tr>
</tbody></table>
...
type,
updatedAt
FROM azure.servicebus.subscriptions
...
```sql
INSERT INTO azure.servicebus.subscriptions (
properties,
resource_group_name,
namespace_name,
topic_name,
...
```sql
REPLACE azure.servicebus.subscriptions
SET 
...
```sql
DELETE FROM azure.servicebus.subscriptions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND namespace_name = '{{ namespace_name }}' --required
AND topic_name = '{{ topic_name }}' --required
AND subscription_name = '{{ subscription_name }}' --required
