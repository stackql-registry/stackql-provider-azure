...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.customproviders.associations" /></td></tr>
</tbody></table>
...
targetResourceId,
type
FROM azure.customproviders.associations
...
```sql
INSERT INTO azure.customproviders.associations (
properties,
scope,
association_name
)
...
```sql
REPLACE azure.customproviders.associations
SET 
...
```sql
DELETE FROM azure.customproviders.associations
WHERE scope = '{{ scope }}' --required
AND association_name = '{{ association_name }}' --required
;
```
