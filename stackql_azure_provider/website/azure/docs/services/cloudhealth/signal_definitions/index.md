...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.cloudhealth.signal_definitions" /></td></tr>
</tbody></table>
...
tags,
type
FROM azure.cloudhealth.signal_definitions
...
```sql
INSERT INTO azure.cloudhealth.signal_definitions (
properties,
resource_group_name,
health_model_name,
...
```sql
REPLACE azure.cloudhealth.signal_definitions
SET 
...
```sql
DELETE FROM azure.cloudhealth.signal_definitions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND health_model_name = '{{ health_model_name }}' --required
AND signal_definition_name = '{{ signal_definition_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
