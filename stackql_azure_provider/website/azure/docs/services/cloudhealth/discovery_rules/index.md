...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.cloudhealth.discovery_rules" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.cloudhealth.discovery_rules
...
```sql
INSERT INTO azure.cloudhealth.discovery_rules (
properties,
resource_group_name,
health_model_name,
discovery_rule_name,
...
```sql
REPLACE azure.cloudhealth.discovery_rules
SET 
...
```sql
DELETE FROM azure.cloudhealth.discovery_rules
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND health_model_name = '{{ health_model_name }}' --required
AND discovery_rule_name = '{{ discovery_rule_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
