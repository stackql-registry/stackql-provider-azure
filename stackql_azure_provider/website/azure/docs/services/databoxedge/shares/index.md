...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.databoxedge.shares" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.databoxedge.shares (
properties,
device_name,
name,
resource_group_name,
...
```sql
REPLACE azure.databoxedge.shares
SET 
...
```sql
DELETE FROM azure.databoxedge.shares
WHERE device_name = '{{ device_name }}' --required
AND name = '{{ name }}' --required
AND resource_group_name = '{{ resource_group_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure.databoxedge.shares.refresh 
@device_name='{{ device_name }}' --required, 
@name='{{ name }}' --required, 
