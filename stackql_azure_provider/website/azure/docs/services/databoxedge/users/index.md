...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.databoxedge.users" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.databoxedge.users (
properties,
device_name,
name,
resource_group_name,
...
```sql
REPLACE azure.databoxedge.users
SET 
...
```sql
DELETE FROM azure.databoxedge.users
WHERE device_name = '{{ device_name }}' --required
AND name = '{{ name }}' --required
AND resource_group_name = '{{ resource_group_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
