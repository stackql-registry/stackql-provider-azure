...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.media.tracks" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.media.tracks (
properties,
resource_group_name,
account_name,
asset_name,
...
```sql
DELETE FROM azure.media.tracks
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND account_name = '{{ account_name }}' --required
AND asset_name = '{{ asset_name }}' --required
AND track_name = '{{ track_name }}' --required
...
```sql
EXEC azure.media.tracks.update_track_data 
@resource_group_name='{{ resource_group_name }}' --required, 
