...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.iotoperations.broker_authorization" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.iotoperations.broker_authorization
...
```sql
INSERT INTO azure.iotoperations.broker_authorization (
properties,
extendedLocation,
resource_group_name,
instance_name,
...
```sql
REPLACE azure.iotoperations.broker_authorization
SET 
...
```sql
DELETE FROM azure.iotoperations.broker_authorization
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND instance_name = '{{ instance_name }}' --required
AND broker_name = '{{ broker_name }}' --required
AND authorization_name = '{{ authorization_name }}' --required
