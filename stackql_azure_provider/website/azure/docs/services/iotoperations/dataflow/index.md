...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.iotoperations.dataflow" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.iotoperations.dataflow
...
```sql
INSERT INTO azure.iotoperations.dataflow (
properties,
extendedLocation,
resource_group_name,
instance_name,
...
```sql
REPLACE azure.iotoperations.dataflow
SET 
...
```sql
DELETE FROM azure.iotoperations.dataflow
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND instance_name = '{{ instance_name }}' --required
AND dataflow_profile_name = '{{ dataflow_profile_name }}' --required
AND dataflow_name = '{{ dataflow_name }}' --required
