...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerregistry.pipeline_runs" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.containerregistry.pipeline_runs
...
```sql
INSERT INTO azure.containerregistry.pipeline_runs (
properties,
resource_group_name,
registry_name,
pipeline_run_name,
...
```sql
DELETE FROM azure.containerregistry.pipeline_runs
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND registry_name = '{{ registry_name }}' --required
AND pipeline_run_name = '{{ pipeline_run_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
