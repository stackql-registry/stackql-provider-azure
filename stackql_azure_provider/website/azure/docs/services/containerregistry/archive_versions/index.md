...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerregistry.archive_versions" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.containerregistry.archive_versions
...
```sql
INSERT INTO azure.containerregistry.archive_versions (
resource_group_name,
registry_name,
package_type,
archive_name,
...
```sql
DELETE FROM azure.containerregistry.archive_versions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND registry_name = '{{ registry_name }}' --required
AND package_type = '{{ package_type }}' --required
AND archive_name = '{{ archive_name }}' --required
