...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerregistry.scope_maps" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.containerregistry.scope_maps
...
```sql
INSERT INTO azure.containerregistry.scope_maps (
properties,
resource_group_name,
registry_name,
scope_map_name,
...
```sql
UPDATE azure.containerregistry.scope_maps
SET 
...
```sql
DELETE FROM azure.containerregistry.scope_maps
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND registry_name = '{{ registry_name }}' --required
AND scope_map_name = '{{ scope_map_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
