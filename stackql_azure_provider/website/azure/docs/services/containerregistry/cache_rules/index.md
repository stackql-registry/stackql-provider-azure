...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerregistry.cache_rules" /></td></tr>
</tbody></table>
...
targetRepository,
type
FROM azure.containerregistry.cache_rules
...
```sql
INSERT INTO azure.containerregistry.cache_rules (
properties,
identity,
resource_group_name,
registry_name,
...
```sql
UPDATE azure.containerregistry.cache_rules
SET 
...
```sql
DELETE FROM azure.containerregistry.cache_rules
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND registry_name = '{{ registry_name }}' --required
AND cache_rule_name = '{{ cache_rule_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
