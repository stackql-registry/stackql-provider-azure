...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerregistry.archives" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.containerregistry.archives
...
```sql
INSERT INTO azure.containerregistry.archives (
properties,
resource_group_name,
registry_name,
package_type,
...
```sql
UPDATE azure.containerregistry.archives
SET 
...
```sql
DELETE FROM azure.containerregistry.archives
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND registry_name = '{{ registry_name }}' --required
AND package_type = '{{ package_type }}' --required
AND archive_name = '{{ archive_name }}' --required
