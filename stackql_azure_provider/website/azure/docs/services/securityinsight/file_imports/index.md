...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.securityinsight.file_imports" /></td></tr>
</tbody></table>
...
type,
validRecordCount
FROM azure.securityinsight.file_imports
...
```sql
INSERT INTO azure.securityinsight.file_imports (
properties,
resource_group_name,
workspace_name,
file_import_id,
...
```sql
DELETE FROM azure.securityinsight.file_imports
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND workspace_name = '{{ workspace_name }}' --required
AND file_import_id = '{{ file_import_id }}' --required
AND subscription_id = '{{ subscription_id }}' --required
