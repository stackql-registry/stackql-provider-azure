...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.securityinsight.incident_tasks" /></td></tr>
</tbody></table>
...
title,
type
FROM azure.securityinsight.incident_tasks
...
```sql
INSERT INTO azure.securityinsight.incident_tasks (
properties,
etag,
resource_group_name,
workspace_name,
...
```sql
REPLACE azure.securityinsight.incident_tasks
SET 
...
```sql
DELETE FROM azure.securityinsight.incident_tasks
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND workspace_name = '{{ workspace_name }}' --required
AND incident_id = '{{ incident_id }}' --required
AND incident_task_id = '{{ incident_task_id }}' --required
