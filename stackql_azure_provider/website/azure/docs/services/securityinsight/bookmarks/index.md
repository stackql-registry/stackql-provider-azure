...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.securityinsight.bookmarks" /></td></tr>
</tbody></table>
...
updated,
updatedBy
FROM azure.securityinsight.bookmarks
...
```sql
INSERT INTO azure.securityinsight.bookmarks (
properties,
etag,
resource_group_name,
workspace_name,
...
```sql
REPLACE azure.securityinsight.bookmarks
SET 
...
```sql
DELETE FROM azure.securityinsight.bookmarks
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND workspace_name = '{{ workspace_name }}' --required
AND bookmark_id = '{{ bookmark_id }}' --required
AND subscription_id = '{{ subscription_id }}' --required
