...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.avs.iscsi_paths" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.avs.iscsi_paths (
properties,
resource_group_name,
private_cloud_name,
subscription_id
...
```sql
DELETE FROM azure.avs.iscsi_paths
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_cloud_name = '{{ private_cloud_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
...
```sql
EXEC azure.avs.iscsi_paths.list_by_private_cloud 
@resource_group_name='{{ resource_group_name }}' --required, 
