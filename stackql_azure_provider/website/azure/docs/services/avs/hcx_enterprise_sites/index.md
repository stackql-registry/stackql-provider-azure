...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.avs.hcx_enterprise_sites" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.avs.hcx_enterprise_sites
...
```sql
INSERT INTO azure.avs.hcx_enterprise_sites (
properties,
resource_group_name,
private_cloud_name,
...
```sql
REPLACE azure.avs.hcx_enterprise_sites
SET 
...
```sql
DELETE FROM azure.avs.hcx_enterprise_sites
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_cloud_name = '{{ private_cloud_name }}' --required
AND hcx_enterprise_site_name = '{{ hcx_enterprise_site_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
