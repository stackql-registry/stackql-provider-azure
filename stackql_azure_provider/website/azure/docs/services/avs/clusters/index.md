...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.avs.clusters" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.avs.clusters (
properties,
sku,
resource_group_name,
private_cloud_name,
...
```sql
DELETE FROM azure.avs.clusters
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND private_cloud_name = '{{ private_cloud_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure.avs.clusters.list_zones 
@resource_group_name='{{ resource_group_name }}' --required, 
