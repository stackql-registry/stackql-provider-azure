...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.eventgrid.permission_bindings" /></td></tr>
</tbody></table>
...
topicSpaceName,
type
FROM azure.eventgrid.permission_bindings
...
```sql
INSERT INTO azure.eventgrid.permission_bindings (
properties,
resource_group_name,
namespace_name,
...
```sql
REPLACE azure.eventgrid.permission_bindings
SET 
...
```sql
DELETE FROM azure.eventgrid.permission_bindings
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND namespace_name = '{{ namespace_name }}' --required
AND permission_binding_name = '{{ permission_binding_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
