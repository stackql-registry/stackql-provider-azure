...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.eventgrid.domain_topic_event_subscriptions" /></td></tr>
</tbody></table>
...
topic,
type
FROM azure.eventgrid.domain_topic_event_subscriptions
...
```sql
INSERT INTO azure.eventgrid.domain_topic_event_subscriptions (
properties,
resource_group_name,
domain_name,
topic_name,
...
```sql
UPDATE azure.eventgrid.domain_topic_event_subscriptions
SET 
...
```sql
REPLACE azure.eventgrid.domain_topic_event_subscriptions
SET 
...
```sql
DELETE FROM azure.eventgrid.domain_topic_event_subscriptions
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND domain_name = '{{ domain_name }}' --required
AND topic_name = '{{ topic_name }}' --required
AND event_subscription_name = '{{ event_subscription_name }}' --required
...
```sql
EXEC azure.eventgrid.domain_topic_event_subscriptions.get_delivery_attributes 
@resource_group_name='{{ resource_group_name }}' --required, 
...
```sql
EXEC azure.eventgrid.domain_topic_event_subscriptions.get_full_url 
@resource_group_name='{{ resource_group_name }}' --required, 
@domain_name='{{ domain_name }}' --required, 
@topic_name='{{ topic_name }}' --required, 
@event_subscription_name='{{ event_subscription_name }}' --required, 
