...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workload_orchestration.site_references" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.workload_orchestration.site_references
...
```sql
INSERT INTO azure.workload_orchestration.site_references (
properties,
resource_group_name,
context_name,
site_reference_name,
...
```sql
UPDATE azure.workload_orchestration.site_references
SET 
...
```sql
REPLACE azure.workload_orchestration.site_references
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure.workload_orchestration.site_references
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND context_name = '{{ context_name }}' --required
AND site_reference_name = '{{ site_reference_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
