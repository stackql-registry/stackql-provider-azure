...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.workload_orchestration.instances" /></td></tr>
</tbody></table>
...
targetId,
type
FROM azure.workload_orchestration.instances
...
```sql
INSERT INTO azure.workload_orchestration.instances (
properties,
extendedLocation,
resource_group_name,
target_name,
...
```sql
UPDATE azure.workload_orchestration.instances
SET 
...
```sql
REPLACE azure.workload_orchestration.instances
SET 
properties = '{{ properties }}',
extendedLocation = '{{ extendedLocation }}'
WHERE 
...
```sql
DELETE FROM azure.workload_orchestration.instances
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND target_name = '{{ target_name }}' --required
AND solution_name = '{{ solution_name }}' --required
AND instance_name = '{{ instance_name }}' --required
