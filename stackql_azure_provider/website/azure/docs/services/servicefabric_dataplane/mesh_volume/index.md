...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.servicefabric_dataplane.mesh_volume" /></td></tr>
</tbody></table>
...
status,
statusDetails
FROM azure.servicefabric_dataplane.mesh_volume
...
```sql
INSERT INTO azure.servicefabric_dataplane.mesh_volume (
name,
properties,
...
```sql
REPLACE azure.servicefabric_dataplane.mesh_volume
SET 
name = '{{ name }}',
...
```sql
DELETE FROM azure.servicefabric_dataplane.mesh_volume
WHERE volume_resource_name = '{{ volume_resource_name }}' --required
AND endpoint = '{{ endpoint }}' --required
;
```
