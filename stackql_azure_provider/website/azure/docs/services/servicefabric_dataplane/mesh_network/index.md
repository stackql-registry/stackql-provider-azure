...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.servicefabric_dataplane.mesh_network" /></td></tr>
</tbody></table>
...
status,
statusDetails
FROM azure.servicefabric_dataplane.mesh_network
...
```sql
INSERT INTO azure.servicefabric_dataplane.mesh_network (
name,
properties,
...
```sql
REPLACE azure.servicefabric_dataplane.mesh_network
SET 
name = '{{ name }}',
...
```sql
DELETE FROM azure.servicefabric_dataplane.mesh_network
WHERE network_resource_name = '{{ network_resource_name }}' --required
AND endpoint = '{{ endpoint }}' --required
;
```
