...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.storagemover.endpoints" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.storagemover.endpoints
...
```sql
INSERT INTO azure.storagemover.endpoints (
properties,
identity,
resource_group_name,
storage_mover_name,
...
```sql
UPDATE azure.storagemover.endpoints
SET 
...
```sql
REPLACE azure.storagemover.endpoints
SET 
properties = '{{ properties }}',
identity = '{{ identity }}'
WHERE 
...
```sql
DELETE FROM azure.storagemover.endpoints
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND storage_mover_name = '{{ storage_mover_name }}' --required
AND endpoint_name = '{{ endpoint_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
