...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.managementgroups.hierarchy_settings" /></td></tr>
</tbody></table>
...
tenantId,
type
FROM azure.managementgroups.hierarchy_settings
...
```sql
INSERT INTO azure.managementgroups.hierarchy_settings (
properties,
group_id
)
SELECT 
...
```sql
UPDATE azure.managementgroups.hierarchy_settings
SET 
...
```sql
REPLACE azure.managementgroups.hierarchy_settings
SET 
properties = '{{ properties }}'
WHERE 
group_id = '{{ group_id }}' --required
...
```sql
DELETE FROM azure.managementgroups.hierarchy_settings
WHERE group_id = '{{ group_id }}' --required
;
```
</TabItem>
...
```sql
EXEC azure.managementgroups.hierarchy_settings.list_raw 
@group_id='{{ group_id }}' --required
;
```
</TabItem>
