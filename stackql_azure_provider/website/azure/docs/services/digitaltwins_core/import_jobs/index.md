...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.digitaltwins_core.import_jobs" /></td></tr>
</tbody></table>
...
purgeDateTime,
status
FROM azure.digitaltwins_core.import_jobs
...
```sql
DELETE FROM azure.digitaltwins_core.import_jobs
WHERE id = '{{ id }}' --required
AND endpoint = '{{ endpoint }}' --required
AND traceparent = '{{ traceparent }}'
AND tracestate = '{{ tracestate }}'
...
```sql
EXEC azure.digitaltwins_core.import_jobs.add 
@id='{{ id }}' --required, 
...
```sql
EXEC azure.digitaltwins_core.import_jobs.cancel 
@id='{{ id }}' --required, 
@endpoint='{{ endpoint }}' --required, 
@traceparent='{{ traceparent }}', 
@tracestate='{{ tracestate }}' 
