...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.sitemanager.sites_by_service_group" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.sitemanager.sites_by_service_group
...
```sql
INSERT INTO azure.sitemanager.sites_by_service_group (
properties,
servicegroup_name,
site_name
)
...
```sql
UPDATE azure.sitemanager.sites_by_service_group
SET 
...
```sql
REPLACE azure.sitemanager.sites_by_service_group
SET 
properties = '{{ properties }}'
WHERE 
servicegroup_name = '{{ servicegroup_name }}' --required
...
```sql
DELETE FROM azure.sitemanager.sites_by_service_group
WHERE servicegroup_name = '{{ servicegroup_name }}' --required
AND site_name = '{{ site_name }}' --required
;
```
