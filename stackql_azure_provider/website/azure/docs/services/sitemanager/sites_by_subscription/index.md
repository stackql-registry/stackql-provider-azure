...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.sitemanager.sites_by_subscription" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.sitemanager.sites_by_subscription
...
```sql
INSERT INTO azure.sitemanager.sites_by_subscription (
properties,
site_name,
subscription_id
)
...
```sql
UPDATE azure.sitemanager.sites_by_subscription
SET 
...
```sql
REPLACE azure.sitemanager.sites_by_subscription
SET 
properties = '{{ properties }}'
WHERE 
site_name = '{{ site_name }}' --required
...
```sql
DELETE FROM azure.sitemanager.sites_by_subscription
WHERE site_name = '{{ site_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
```
