...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.trafficmanager.profiles" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.trafficmanager.profiles (
id,
name,
type,
tags,
...
```sql
UPDATE azure.trafficmanager.profiles
SET 
id = '{{ id }}',
name = '{{ name }}',
type = '{{ type }}',
...
```sql
REPLACE azure.trafficmanager.profiles
SET 
id = '{{ id }}',
name = '{{ name }}',
type = '{{ type }}',
...
```sql
DELETE FROM azure.trafficmanager.profiles
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND profile_name = '{{ profile_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
...
```sql
EXEC azure.trafficmanager.profiles.check_traffic_manager_name_availability_v2 
