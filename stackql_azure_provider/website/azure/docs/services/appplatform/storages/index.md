...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.appplatform.storages" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.appplatform.storages
...
```sql
INSERT INTO azure.appplatform.storages (
properties,
resource_group_name,
service_name,
storage_name,
...
```sql
REPLACE azure.appplatform.storages
SET 
...
```sql
DELETE FROM azure.appplatform.storages
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND storage_name = '{{ storage_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
