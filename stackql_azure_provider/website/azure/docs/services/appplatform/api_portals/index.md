...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.appplatform.api_portals" /></td></tr>
</tbody></table>
...
type,
url
FROM azure.appplatform.api_portals
...
```sql
INSERT INTO azure.appplatform.api_portals (
properties,
sku,
resource_group_name,
service_name,
...
```sql
REPLACE azure.appplatform.api_portals
SET 
...
```sql
DELETE FROM azure.appplatform.api_portals
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND api_portal_name = '{{ api_portal_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure.appplatform.api_portals.validate_domain 
@resource_group_name='{{ resource_group_name }}' --required, 
