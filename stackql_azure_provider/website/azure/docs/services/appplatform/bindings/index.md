...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.appplatform.bindings" /></td></tr>
</tbody></table>
...
type,
updatedAt
FROM azure.appplatform.bindings
...
```sql
INSERT INTO azure.appplatform.bindings (
properties,
resource_group_name,
service_name,
app_name,
...
```sql
UPDATE azure.appplatform.bindings
SET 
...
```sql
REPLACE azure.appplatform.bindings
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure.appplatform.bindings
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND app_name = '{{ app_name }}' --required
AND binding_name = '{{ binding_name }}' --required
