...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.iot_deviceprovisioning.enrollment" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO azure.iot_deviceprovisioning.enrollment (
id,
endpoint,
If-Match
)
...
```sql
REPLACE azure.iot_deviceprovisioning.enrollment
SET 
...
```sql
DELETE FROM azure.iot_deviceprovisioning.enrollment
...
```sql
EXEC azure.iot_deviceprovisioning.enrollment.get_raw 
@id='{{ id }}' --required, 
...
```sql
EXEC azure.iot_deviceprovisioning.enrollment.get_attestation_mechanism 
@id='{{ id }}' --required, 
@endpoint='{{ endpoint }}' --required
;
```
...
```sql
EXEC azure.iot_deviceprovisioning.enrollment.run_bulk_operation 
@endpoint='{{ endpoint }}' --required
;
```
</TabItem>
