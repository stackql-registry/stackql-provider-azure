...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.connected_vmware.datastores" /></td></tr>
</tbody></table>
...
uuid,
vCenterId
FROM azure.connected_vmware.datastores
...
```sql
INSERT INTO azure.connected_vmware.datastores (
location,
extendedLocation,
tags,
kind,
...
```sql
UPDATE azure.connected_vmware.datastores
SET 
tags = '{{ tags }}'
WHERE 
...
```sql
DELETE FROM azure.connected_vmware.datastores
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND datastore_name = '{{ datastore_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
AND force = '{{ force }}'
