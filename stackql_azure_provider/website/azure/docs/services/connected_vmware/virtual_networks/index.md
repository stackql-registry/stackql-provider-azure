...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.connected_vmware.virtual_networks" /></td></tr>
</tbody></table>
...
uuid,
vCenterId
FROM azure.connected_vmware.virtual_networks
...
```sql
INSERT INTO azure.connected_vmware.virtual_networks (
location,
extendedLocation,
tags,
kind,
...
```sql
UPDATE azure.connected_vmware.virtual_networks
SET 
tags = '{{ tags }}'
WHERE 
...
```sql
DELETE FROM azure.connected_vmware.virtual_networks
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND virtual_network_name = '{{ virtual_network_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
AND force = '{{ force }}'
