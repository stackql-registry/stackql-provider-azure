...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.connected_vmware.clusters" /></td></tr>
</tbody></table>
...
uuid,
vCenterId
FROM azure.connected_vmware.clusters
...
```sql
INSERT INTO azure.connected_vmware.clusters (
location,
extendedLocation,
tags,
kind,
...
```sql
UPDATE azure.connected_vmware.clusters
SET 
tags = '{{ tags }}'
WHERE 
...
```sql
DELETE FROM azure.connected_vmware.clusters
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
AND force = '{{ force }}'
