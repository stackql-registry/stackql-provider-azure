...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.datamigration.files" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.datamigration.files
...
```sql
INSERT INTO azure.datamigration.files (
properties,
etag,
group_name,
service_name,
...
```sql
UPDATE azure.datamigration.files
SET 
...
```sql
REPLACE azure.datamigration.files
SET 
properties = '{{ properties }}',
etag = '{{ etag }}'
WHERE 
...
```sql
DELETE FROM azure.datamigration.files
WHERE group_name = '{{ group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND project_name = '{{ project_name }}' --required
AND file_name = '{{ file_name }}' --required
...
```sql
EXEC azure.datamigration.files.read 
@group_name='{{ group_name }}' --required, 
...
```sql
EXEC azure.datamigration.files.read_write 
@group_name='{{ group_name }}' --required, 
@service_name='{{ service_name }}' --required, 
@project_name='{{ project_name }}' --required, 
@file_name='{{ file_name }}' --required, 
