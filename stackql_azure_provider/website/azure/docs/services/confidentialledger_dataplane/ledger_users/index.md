...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.confidentialledger_dataplane.ledger_users" /></td></tr>
</tbody></table>
...
assignedRoles,
userId
FROM azure.confidentialledger_dataplane.ledger_users
...
```sql
INSERT INTO azure.confidentialledger_dataplane.ledger_users (
assignedRoles,
user_id,
ledger_endpoint
)
...
```sql
REPLACE azure.confidentialledger_dataplane.ledger_users
SET 
...
```sql
DELETE FROM azure.confidentialledger_dataplane.ledger_users
WHERE user_id = '{{ user_id }}' --required
AND ledger_endpoint = '{{ ledger_endpoint }}' --required
;
```
