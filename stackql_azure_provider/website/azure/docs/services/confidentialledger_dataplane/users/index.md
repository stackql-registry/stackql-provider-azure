...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.confidentialledger_dataplane.users" /></td></tr>
</tbody></table>
...
assignedRole,
userId
FROM azure.confidentialledger_dataplane.users
...
```sql
INSERT INTO azure.confidentialledger_dataplane.users (
assignedRole,
user_id,
ledger_endpoint
)
...
```sql
REPLACE azure.confidentialledger_dataplane.users
SET 
...
```sql
DELETE FROM azure.confidentialledger_dataplane.users
WHERE user_id = '{{ user_id }}' --required
AND ledger_endpoint = '{{ ledger_endpoint }}' --required
;
```
