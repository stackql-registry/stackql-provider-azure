...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.resiliencemanagement.goal_templates" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.resiliencemanagement.goal_templates
...
```sql
INSERT INTO azure.resiliencemanagement.goal_templates (
properties,
service_group_name,
goal_template_name
)
...
```sql
UPDATE azure.resiliencemanagement.goal_templates
SET 
...
```sql
REPLACE azure.resiliencemanagement.goal_templates
SET 
properties = '{{ properties }}'
WHERE 
service_group_name = '{{ service_group_name }}' --required
...
```sql
DELETE FROM azure.resiliencemanagement.goal_templates
WHERE service_group_name = '{{ service_group_name }}' --required
AND goal_template_name = '{{ goal_template_name }}' --required
;
```
