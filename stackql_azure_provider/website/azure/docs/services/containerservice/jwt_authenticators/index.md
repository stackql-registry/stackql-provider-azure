...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.containerservice.jwt_authenticators" /></td></tr>
</tbody></table>
...
type,
userValidationRules
FROM azure.containerservice.jwt_authenticators
...
```sql
INSERT INTO azure.containerservice.jwt_authenticators (
properties,
resource_group_name,
resource_name,
...
```sql
REPLACE azure.containerservice.jwt_authenticators
SET 
...
```sql
DELETE FROM azure.containerservice.jwt_authenticators
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND resource_name = '{{ resource_name }}' --required
AND jwt_authenticator_name = '{{ jwt_authenticator_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
