...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.timeseriesinsights.access_policies" /></td></tr>
</tbody></table>
...
roles,
type
FROM azure.timeseriesinsights.access_policies
...
```sql
INSERT INTO azure.timeseriesinsights.access_policies (
properties,
resource_group_name,
environment_name,
access_policy_name,
...
```sql
UPDATE azure.timeseriesinsights.access_policies
SET 
...
```sql
REPLACE azure.timeseriesinsights.access_policies
SET 
properties = '{{ properties }}'
WHERE 
resource_group_name = '{{ resource_group_name }}' --required
...
```sql
DELETE FROM azure.timeseriesinsights.access_policies
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND environment_name = '{{ environment_name }}' --required
AND access_policy_name = '{{ access_policy_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
