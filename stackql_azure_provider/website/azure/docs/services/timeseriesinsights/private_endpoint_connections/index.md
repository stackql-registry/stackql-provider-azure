...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.timeseriesinsights.private_endpoint_connections" /></td></tr>
</tbody></table>
...
provisioningState,
type
FROM azure.timeseriesinsights.private_endpoint_connections
...
```sql
INSERT INTO azure.timeseriesinsights.private_endpoint_connections (
properties,
resource_group_name,
environment_name,
...
```sql
REPLACE azure.timeseriesinsights.private_endpoint_connections
SET 
...
```sql
DELETE FROM azure.timeseriesinsights.private_endpoint_connections
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND environment_name = '{{ environment_name }}' --required
AND private_endpoint_connection_name = '{{ private_endpoint_connection_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
