...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.redis_enterprise.redis_enterprise" /></td></tr>
</tbody></table>
...
type,
zones
FROM azure.redis_enterprise.redis_enterprise
...
```sql
INSERT INTO azure.redis_enterprise.redis_enterprise (
tags,
location,
properties,
sku,
...
```sql
UPDATE azure.redis_enterprise.redis_enterprise
SET 
sku = '{{ sku }}',
...
```sql
DELETE FROM azure.redis_enterprise.redis_enterprise
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
...
```sql
EXEC azure.redis_enterprise.redis_enterprise.list_skus_for_scaling 
@resource_group_name='{{ resource_group_name }}' --required, 
