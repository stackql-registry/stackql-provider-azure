...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.rdbms.private_endpoint_connections" /></td></tr>
</tbody></table>
...
provisioningState,
type
FROM azure.rdbms.private_endpoint_connections
...
```sql
INSERT INTO azure.rdbms.private_endpoint_connections (
properties,
resource_group_name,
server_name,
...
```sql
UPDATE azure.rdbms.private_endpoint_connections
SET 
tags = '{{ tags }}'
WHERE 
...
```sql
REPLACE azure.rdbms.private_endpoint_connections
SET 
...
```sql
DELETE FROM azure.rdbms.private_endpoint_connections
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND server_name = '{{ server_name }}' --required
AND private_endpoint_connection_name = '{{ private_endpoint_connection_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
