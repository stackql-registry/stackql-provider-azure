...
type,
virtualMachineState
FROM azure.standbypool.standby_virtual_machine_pools
...
```sql
INSERT INTO azure.standbypool.standby_virtual_machine_pools (
tags,
location,
properties,
resource_group_name,
...
```sql
UPDATE azure.standbypool.standby_virtual_machine_pools
SET 
tags = '{{ tags }}',
...
```sql
REPLACE azure.standbypool.standby_virtual_machine_pools
SET 
tags = '{{ tags }}',
location = '{{ location }}',
properties = '{{ properties }}'
...
```sql
DELETE FROM azure.standbypool.standby_virtual_machine_pools
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND standby_virtual_machine_pool_name = '{{ standby_virtual_machine_pool_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
;
