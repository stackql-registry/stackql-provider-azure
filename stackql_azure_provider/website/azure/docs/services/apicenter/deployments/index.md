...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.apicenter.deployments" /></td></tr>
</tbody></table>
...
title,
type
FROM azure.apicenter.deployments
...
```sql
INSERT INTO azure.apicenter.deployments (
properties,
resource_group_name,
service_name,
workspace_name,
...
```sql
REPLACE azure.apicenter.deployments
SET 
...
```sql
DELETE FROM azure.apicenter.deployments
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND service_name = '{{ service_name }}' --required
AND workspace_name = '{{ workspace_name }}' --required
AND api_name = '{{ api_name }}' --required
...
```sql
EXEC azure.apicenter.deployments.head 
@resource_group_name='{{ resource_group_name }}' --required, 
