...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.eventhub.schema_registry" /></td></tr>
</tbody></table>
...
type,
updatedAtUtc
FROM azure.eventhub.schema_registry
...
```sql
INSERT INTO azure.eventhub.schema_registry (
properties,
resource_group_name,
namespace_name,
schema_group_name,
...
```sql
REPLACE azure.eventhub.schema_registry
SET 
...
```sql
DELETE FROM azure.eventhub.schema_registry
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND namespace_name = '{{ namespace_name }}' --required
AND schema_group_name = '{{ schema_group_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
