...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.cognitiveservices.account_capability_hosts" /></td></tr>
</tbody></table>
...
FROM azure.cognitiveservices.account_capability_hosts
...
```sql
INSERT INTO azure.cognitiveservices.account_capability_hosts (
properties,
resource_group_name,
account_name,
...
```sql
REPLACE azure.cognitiveservices.account_capability_hosts
SET 
...
```sql
DELETE FROM azure.cognitiveservices.account_capability_hosts
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND account_name = '{{ account_name }}' --required
AND capability_host_name = '{{ capability_host_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
