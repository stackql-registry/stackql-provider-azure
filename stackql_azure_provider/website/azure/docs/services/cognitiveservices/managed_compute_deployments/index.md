...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.cognitiveservices.managed_compute_deployments" /></td></tr>
</tbody></table>
...
FROM azure.cognitiveservices.managed_compute_deployments
...
```sql
INSERT INTO azure.cognitiveservices.managed_compute_deployments (
properties,
sku,
resource_group_name,
account_name,
...
```sql
UPDATE azure.cognitiveservices.managed_compute_deployments
SET 
sku = '{{ sku }}'
WHERE 
...
```sql
REPLACE azure.cognitiveservices.managed_compute_deployments
SET 
...
```sql
DELETE FROM azure.cognitiveservices.managed_compute_deployments
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND account_name = '{{ account_name }}' --required
AND deployment_name = '{{ deployment_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
