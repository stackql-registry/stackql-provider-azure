...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.cosmosdbforpostgresql.roles" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.cosmosdbforpostgresql.roles
...
```sql
INSERT INTO azure.cosmosdbforpostgresql.roles (
properties,
resource_group_name,
cluster_name,
role_name,
...
```sql
DELETE FROM azure.cosmosdbforpostgresql.roles
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND cluster_name = '{{ cluster_name }}' --required
AND role_name = '{{ role_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
