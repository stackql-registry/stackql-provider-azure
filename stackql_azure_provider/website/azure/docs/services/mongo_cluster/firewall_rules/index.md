...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.mongo_cluster.firewall_rules" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.mongo_cluster.firewall_rules
...
```sql
INSERT INTO azure.mongo_cluster.firewall_rules (
properties,
resource_group_name,
mongo_cluster_name,
firewall_rule_name,
...
```sql
REPLACE azure.mongo_cluster.firewall_rules
SET 
...
```sql
DELETE FROM azure.mongo_cluster.firewall_rules
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND mongo_cluster_name = '{{ mongo_cluster_name }}' --required
AND firewall_rule_name = '{{ firewall_rule_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
