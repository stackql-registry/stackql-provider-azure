...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.mysqlflexibleservers.private_endpoint_connections" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.mysqlflexibleservers.private_endpoint_connections
...
```sql
INSERT INTO azure.mysqlflexibleservers.private_endpoint_connections (
properties,
resource_group_name,
server_name,
...
```sql
REPLACE azure.mysqlflexibleservers.private_endpoint_connections
SET 
...
```sql
DELETE FROM azure.mysqlflexibleservers.private_endpoint_connections
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND server_name = '{{ server_name }}' --required
AND private_endpoint_connection_name = '{{ private_endpoint_connection_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
