...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.mysqlflexibleservers.databases" /></td></tr>
</tbody></table>
...
systemData,
type
FROM azure.mysqlflexibleservers.databases
...
```sql
INSERT INTO azure.mysqlflexibleservers.databases (
properties,
resource_group_name,
server_name,
database_name,
...
```sql
REPLACE azure.mysqlflexibleservers.databases
SET 
...
```sql
DELETE FROM azure.mysqlflexibleservers.databases
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND server_name = '{{ server_name }}' --required
AND database_name = '{{ database_name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
