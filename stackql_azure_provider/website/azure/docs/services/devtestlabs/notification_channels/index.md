...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.devtestlabs.notification_channels" /></td></tr>
</tbody></table>
...
uniqueIdentifier,
webHookUrl
FROM azure.devtestlabs.notification_channels
...
```sql
INSERT INTO azure.devtestlabs.notification_channels (
location,
tags,
properties,
resource_group_name,
...
```sql
UPDATE azure.devtestlabs.notification_channels
SET 
...
```sql
REPLACE azure.devtestlabs.notification_channels
SET 
location = '{{ location }}',
tags = '{{ tags }}',
...
```sql
DELETE FROM azure.devtestlabs.notification_channels
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND lab_name = '{{ lab_name }}' --required
AND name = '{{ name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
...
```sql
EXEC azure.devtestlabs.notification_channels.notify 
@resource_group_name='{{ resource_group_name }}' --required, 
