...
<tr><td><b>Id</b></td><td><CopyableCode code="azure.devtestlabs.formulas" /></td></tr>
</tbody></table>
...
uniqueIdentifier,
vm
FROM azure.devtestlabs.formulas
...
```sql
INSERT INTO azure.devtestlabs.formulas (
location,
tags,
properties,
resource_group_name,
...
```sql
UPDATE azure.devtestlabs.formulas
SET 
...
```sql
REPLACE azure.devtestlabs.formulas
SET 
location = '{{ location }}',
tags = '{{ tags }}',
...
```sql
DELETE FROM azure.devtestlabs.formulas
WHERE resource_group_name = '{{ resource_group_name }}' --required
AND lab_name = '{{ lab_name }}' --required
AND name = '{{ name }}' --required
AND subscription_id = '{{ subscription_id }}' --required
